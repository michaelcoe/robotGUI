...
#runs through the process of moving and saving data
def moveq(move):
#    or_h.setSteeringRate(0)
...
#    or_h.setSteeringGains(steeringGains)

    numSamples = or_h.calcNumSamples(move)
    
#    or_h.eraseFlashMem(numSamples)
#    or_h.startTelemetrySave(numSamples)
    
    time.sleep(500 / 1000.0)
    
#    or_h.sendMoveQueue(move)
#		or_h.writeFileHeader(dataFileName)
    time.sleep(0.5)
#    or_h.downloadTelemetry(numSamples)
...
def plotGraph(figure):
    ...
    time = np.loadtxt(shared.dataFileName, unpack=True, usecols=[0])
    Gyrox, Gyroy, Gyroz = np.loadtxt(shared.dataFileName,unpack=True, usecols=[5,6,7])
    XLx, XLy, XLz = np.loadtxt(shared.dataFileName, unpack=True, usecols=[8,9,10])
    LBemf, RBemf = np.loadtxt(shared.dataFileName, unpack=True, usecols=[11,12])
    x = 0
